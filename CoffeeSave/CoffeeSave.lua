...
local json = require('CoffeeSave.json') -- used for HTML5
...
M.verbose = false -- if true then more information will be printed such as autosaves
...
function M.set_data_autosaver(data_autosaver)
    M.data_autosaver = data_autosaver
end
...
function M.get_file_path(file)
    if M.appname == 'defsave' then
        log('DefSave: You need to set a non-default appname to CoffeeSave.appname')
    end
    if file == nil then
...
function M.load(file)
    ...
    if M.loaded[file] ~= nil then
        if M.block_reloading == false then
            log('DefSave: Warning the file ' .. file ..
                    ' was already loaded and will be reloaded possibly overwriting changes')
        else
    ...
    if html5 and web_filestorage then
        local json_value = web_filestorage.load_file(path)
        if json_value then
            loaded_file = json.decode(json_value)
        end
    else
...
function M.save(file, force)
    ...
    if M.loaded[file].changed == false and force == false then
        if M.verbose then
            log("DefSave: File '" .. file ..
                    "' is unchanged so not saving, set force flag to true to force saving if changed flag is false")
        end
        return true
    end
    ...
    if html5 then
        ...
        web_filestorage.save_file(path, encoded_data)
    ...
    if is_save_successful then
        ...
        log("DefSave: Something went wrong when attempting to save the file '" .. tostring(file) .. "' to the path '" ..
                path .. "'")
        return nil
    end
end
...
function M.update(dt)
    if M.autosave == true then
        if dt == nil then
            log('DefSave: You must pass dt to CoffeeSave.update')
        end
        M.timer = M.timer + dt

        if M.timer >= M.autosave_timer then
            if M.changed == true then
                if M.data_autosaver then
                    M.data_autosaver()
                else
                    M.save_all()
                end
